...
func (d *userDao) updateDataByID(ctx context.Context, db *gorm.DB, table *model.User) error {
	...
	if table.TwId != "" {
		update["twId"] = table.TwId
	}
	if table.TwName != "" {
		update["twName"] = table.TwName
	}
	if table.AvatarUrl != "" {
		update["avatarUrl"] = table.AvatarUrl
	}
	if table.Token != "" {
		update["token"] = table.Token
	}
	if table.TokenSecret != "" {
		update["tokenSecret"] = table.TokenSecret
	}
	if table.WalletAddress != "" {
		update["walletAddress"] = table.WalletAddress
	}
	if table.UpdatedAt.IsZero() == false {
		update["updatedAt"] = table.UpdatedAt
	}
	if table.CreatedAt.IsZero() == false {
		update["createdAt"] = table.CreatedAt
	}
	if table.DeletedAt.IsZero() == false {
		update["deletedAt"] = table.DeletedAt
	}
	if table.LikeCount != 0 {
		update["likeCount"] = table.LikeCount
	}
